...
export default function DestinationsManagement() {
        ...
        const channel = supabase
            ...
            .on(
                ...
                (payload) => {
                    ...
                    // Silently refresh without toast to avoid spam
    ...
    const handleToggleFeatured = async (destination: Destination) => {
        ...
        if (error) {
            ...
            toast.success(destination.is_featured ? "Removed from featured" : "Marked as featured");
            fetchDestinations();
        }
    };

    const handleTogglePopular = async (destination: Destination) => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const { error } = await (supabase.from('destinations') as any)
            .update({ is_popular: !destination.is_popular })
            .eq('id', destination.id);

        if (error) {
            toast.error("Update failed: " + error.message);
        } else {
            toast.success(destination.is_popular ? "Removed from popular" : "Marked as popular");
            fetchDestinations();
        }
    };

    const handleViewClick = (destination: Destination) => {
                                    ...
                                    <TableCell className="text-right">
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-52">
                                                ...
                                                <DropdownMenuItem className="gap-2" onClick={() => handleEditClick(item)}>
                                                    <Edit className="h-4 w-4" /> Edit
                                                </DropdownMenuItem>
                                                <DropdownMenuSeparator />
                                                <DropdownMenuItem
                                                    className="gap-2"
                                                    onClick={() => handleUpdateStatus(item.id, item.status === 'published' ? 'draft' : 'published')}
                                                >
                                                ...
                                                {item.status !== 'archived' && (
                                                    <DropdownMenuItem className="gap-2" onClick={() => handleUpdateStatus(item.id, 'archived')}>
