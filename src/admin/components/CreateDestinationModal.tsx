    ...
    const handleImageUpload = useCallback(
        async (event: React.ChangeEvent<HTMLInputElement>) => {
            ...
            try {
                ...
                if (uploadError) {
                    // If bucket doesn't exist, use a placeholder URL approach
                    console.error("Upload error:", uploadError);
                    // Create a local preview and use external URL fallback
                    const reader = new FileReader();
                    reader.onloadend = () => {
                        setImagePreview(reader.result as string);
                        // For now, store base64 or use a placeholder
                        toast.info(
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { error } = await (supabase.from("destinations") as any).insert({
                ...
                estimated_cost: parseFloat(formData.estimated_cost), // Price in KES
        ...
        <Dialog open={open} onOpenChange={handleClose}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Add a new destination to the catalog. Fill in all required fields
                        marked with *.
                    </DialogDescription>
                </DialogHeader>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Select
                                ...
                                onValueChange={(value) =>
                                    setFormData((prev) => ({ ...prev, popularity_badge: value }))
                                }
                            >
                                <SelectTrigger>
