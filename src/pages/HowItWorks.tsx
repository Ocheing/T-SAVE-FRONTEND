          ...
          <div className="space-y-10">
            {steps.map((step, index) => (
              ...
              >
                ...
                <div className="hidden md:flex absolute left-1/2 top-12 -translate-x-1/2 z-10 w-10 h-10 rounded-full bg-background border-2 border-primary items-center justify-center shadow-lg shadow-primary/20">
                ...
                <Card className="p-0 overflow-hidden hover:shadow-[var(--shadow-elegant)] transition-all duration-500 hover:-translate-y-1 border-white/5">
                  <div className={`flex flex-col md:flex-row ${index % 2 === 1 ? 'md:flex-row-reverse' : ''}`}>
                    {/* Visual Side */}
                    <div className={`md:w-2/5 p-10 md:p-14 flex flex-col items-center justify-center bg-gradient-to-br ${step.gradient} relative`}>
                      <span className="absolute top-4 left-4 text-7xl font-black text-primary/8 select-none">
                        {step.number}
                      </span>
                      <div className="w-24 h-24 rounded-3xl bg-primary/15 border border-primary/20 flex items-center justify-center shadow-lg shadow-primary/10">
                    ...
                    <div className="md:w-3/5 p-8 md:p-12 flex flex-col justify-center">
                      {/* Mobile step badge */}
                      <div className="md:hidden inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-primary/10 text-primary text-xs font-bold mb-3 w-fit">
                        Step {step.number}
                      </div>
                      <h3 className="text-2xl md:text-3xl font-bold mb-4">{step.title}</h3>
                      <p className="text-muted-foreground mb-6 leading-relaxed text-base">{step.description}</p>
